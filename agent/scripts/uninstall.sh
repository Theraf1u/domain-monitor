...
echo "Это остановит и удалит контейнер и Docker-образ Domain Monitor Agent."
...
read -r -p "Удалить также локальный буфер событий в ./data? (yes/no): " wipe_data </dev/tty
