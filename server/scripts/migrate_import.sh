...
# spec 10: this replaces the live DB the same way restore.sh does -
# refuse while an automated migrate-to is in progress, checked while
# the API is still up (before the container gets stopped below).
...
if [ -n "$ADMIN_API_KEY" ]; then
    ...
    if [ -n "$active_job" ] && [ "$active_job" != "null" ]; then
        echo "✗ Сейчас выполняется миграция - импорт временно недоступен: $active_job" >&2
        exit 1
    fi
fi
